...
class PluginConfig extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { plugin, pluginStatus, ipcPath } = this.props
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {selectedTab === 4 && (
          <TabContainer>
            <Wallet plugin={plugin} ipcPath={ipcPath} accounts={accounts} />
          </TabContainer>
        )}
        {/* {selectedTab === 5 && (
          <TabContainer>
...
function mapStateToProps(state) {
  ...
  return {
    ...
    ipcPath: state.plugin[selected].ipcPath,
    accounts: state.plugin[selected].active.accounts
  }
}
