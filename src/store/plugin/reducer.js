...
const plugin = (state = initialState, action) => {
  switch (action.type) {
    ...
    case 'PLUGIN:SET_IPC_PATH': {
      const { pluginName, ipcPath } = action.payload
      return {
        ...state,
        [pluginName]: {
          ...initialPluginState,
    ...
    case 'PLUGIN:UPDATE_ACCOUNTS': {
      const { pluginName, accounts } = action.payload
      const activeState = state[pluginName]
        ? state[pluginName].active
        : initialPluginState.active
