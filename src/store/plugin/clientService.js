...
class ClientService {
  ...
  watchForAccounts(plugin, dispatch) {
    this.updateAccounsInterval = setInterval(
      () => this.updateAccounts(plugin, dispatch),
      3000
    )
  }

  async updateAccounts(plugin, dispatch) {
    const accounts = await plugin.rpc('personal_listAccounts')
    if (accounts.message) {
      dispatch(updateAccountsError(plugin.name, accounts.message))
    } else {
      dispatch(updateAccounts(plugin.name, accounts))
    }
  }
  ...
  clearUpdateAccountsInterval() {
    clearInterval(this.updateAccounsInterval)
  }
